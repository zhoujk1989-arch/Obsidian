...
- 本来源包用于沉淀一表通系统 `6.19 代理协议` 的目标表结构证据。
- 本地原始材料已拆分入库：`原始材料/表结构/一表通系统/T_6_19-代理协议-DDL-2026-04-27.sql`。
- 当前按数据表 ingest 初始化字段主档，不硬造报表业务口径页或字段级血缘页。
- 外部来源关联：待补充。

## 本地文件

- `原始材料/表结构/一表通系统/T_6_19-代理协议-DDL-2026-04-27.sql`
...
- 外部知识库：`my-dev-brain`
- 外部关联页面：待补充
- 检索关键词：`一表通`、`6.19`、`代理协议`、`T_6_19`

## Key Findings

- `T_6_19` 表注释为 `6.19代理协议`，本次 DDL 解析字段数为 20。
...
- 需补充 `T_6_19` 的加工 SQL、存储过程或视图定义，确认字段来源、上下游对象和字段级血缘。
