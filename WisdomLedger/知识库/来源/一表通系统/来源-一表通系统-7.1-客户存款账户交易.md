...
- 本来源包用于沉淀一表通系统 `7.1 客户存款账户交易` 的目标表结构证据。
- 本地原始材料已拆分入库：`原始材料/表结构/一表通系统/T_7_1-客户存款账户交易-DDL-2026-04-27.sql`。
...
- 外部知识库：`my-dev-brain`
- 外部关联页面：待补充
- 检索关键词：`一表通`、`7.1`、`客户存款账户交易`、`T_7_1`

## Key Findings

- `T_7_1` 表注释为 `7.1客户存款账户交易`，本次 DDL 解析字段数为 36。
...
- 需补充 `T_7_1` 的加工 SQL、存储过程或视图定义，确认字段来源、上下游对象和字段级血缘。
