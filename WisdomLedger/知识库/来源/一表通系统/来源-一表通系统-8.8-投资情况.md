...
- 本来源包用于沉淀一表通系统 `8.8 投资情况` 的目标表结构证据。
- 本地原始材料已拆分入库：`原始材料/表结构/一表通系统/T_8_8-投资情况-DDL-2026-04-27.sql`。
...
- 外部知识库：`my-dev-brain`
- 外部关联页面：待补充
- 检索关键词：`一表通`、`8.8`、`投资情况`、`T_8_8`

## Key Findings

- `T_8_8` 表注释为 `8.8投资情况`，本次 DDL 解析字段数为 32。
...
- 需补充 `T_8_8` 的加工 SQL、存储过程或视图定义，确认字段来源、上下游对象和字段级血缘。
