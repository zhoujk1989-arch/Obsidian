...
- 本来源包记录 `IE_006_603` `表内外业务抵质押物` 的本地 EAST5.0 表结构 DDL。
...
- `IE_006_603` 的业务名称为 `表内外业务抵质押物`，本地建表注释为“表内外业务抵质押物”。
- DDL 当前包含 `26` 个字段，字段注释中标注 `PK` 的核心标识为：`CJRQ`, `DBHTH`, `YPBH`。
...
```sql
-- 完整 DDL 见 `原始材料/表结构/EAST5.0系统/IE_006_603-表内外业务抵质押物-DDL-2026-04-28.sql`
...
- 当前尚未取得 `IE_006_603` 的实际装载 SQL、存储过程或接口落地脚本，字段级来源和加工状态待补。
