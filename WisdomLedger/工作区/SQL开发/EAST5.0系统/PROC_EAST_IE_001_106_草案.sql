...
CREATE PROCEDURE `PROC_EAST_IE_001_106`(
...
BEGIN
    ...
    -- 上月最后一天：当月第一天减1天
    SET v_prev_month_last = DATE_SUB(DATE_SUB(v_data_date, INTERVAL (DAY(v_data_date) - 1) DAY), INTERVAL 1 DAY);
    ...
    prev_t16 AS (
        SELECT
            ...
            A060017   -- 股东或关联方状态
        FROM T_1_6
        WHERE A060024 = v_prev_month_last
    ),
    ...
    SELECT
        ...
        CASE
            ...
            WHEN cur.A060009 LIKE '00-%' THEN CONCAT('其他-', SUBSTRING(cur.A060009, 4))
        ...
        CASE
            ...
            WHEN cur.A060015 LIKE '00-%' THEN CONCAT('其他-', SUBSTRING(cur.A060015, 4))
        ...
        CASE
            ...
            ELSE DATE_FORMAT(cur.A060020, '%Y%m%d')
        END AS RGRQ,
        ...
        CASE
            ...
            ELSE DATE_FORMAT(cur.A060023, '%Y%m%d')
        END AS ZJYCBDRQ,
        ...
        --     TODO: 确认业务来源
        SUBSTRING(NULLIF(TRIM(cur.A060002), ''), 12) AS GSFZJG,

        -- 16. 内部机构号：从 T_1_6.A060002 第12位截取至最后一位
        SUBSTRING(NULLIF(TRIM(cur.A060002), ''), 12) AS NBJGH,
        ...
        CASE
            ...
            WHEN cur.A060004 LIKE '00-%' THEN CONCAT('其他-', SUBSTRING(cur.A060004, 4))
        ...
        COALESCE(
            ...
            CASE
                WHEN cur.A060005 LIKE '00-%' THEN CONCAT('其他-', SUBSTRING(cur.A060005, 4))
        ...
        CASE
            ...
            WHEN cur.A060017 LIKE '00-%' THEN CONCAT('其他-', SUBSTRING(cur.A060017, 4))
    ...
    LEFT JOIN dim_code_mapping cm
          ...
          AND cm.start_date  <= v_data_date
          AND cm.end_date     > v_data_date
