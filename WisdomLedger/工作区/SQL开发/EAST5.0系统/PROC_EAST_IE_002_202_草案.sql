...
CREATE PROCEDURE `PROC_EAST_IE_002_202`(
...
BEGIN
    ...
    DECLARE V_PROC_NAME VARCHAR(200) DEFAULT 'PROC_EAST_IE_002_202';
    ...
    SET V_PREV_DATE = DATE_SUB(V_DATA_DATE, INTERVAL 1 DAY);
    ...
    -- ============================================================
    -- 2. 主加工区：从 T_3_7 取关系数据，关联 IE_002_201 取客户信息
    -- ============================================================
    SET V_START_DT = NOW();
    SET V_STEP_NO = V_STEP_NO + 1;
    SET V_DESCB = '主加工：映射个人客户关系';
    ...
    SELECT
        ...
        DATE_FORMAT(rel.C070011, '%Y%m%d') AS CJRQ,
    ...
    WHERE rel.C070011 = V_DATA_DATE
      -- 剔除：解除关系日期非空且小于上个采集日期（已在上个周期前解除的关系不再报送）
      AND NOT (rel.C070010 IS NOT NULL AND rel.C070010 < V_PREV_DATE)
      -- 剔除：本人为本人担保的关系不报送
