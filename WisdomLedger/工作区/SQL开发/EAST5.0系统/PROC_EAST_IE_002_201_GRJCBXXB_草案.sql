...
CREATE PROCEDURE `PROC_EAST_IE_002_201_GRJCBXXB`(
    IN I_DATE VARCHAR(8),
...
BEGIN
  ...
  cust_info AS (
      ...
      FROM (
          ...
          FROM T_2_5 t
          WHERE t.B050036 = TO_CHAR(P_DATA_DATE, 'YYYY-MM-DD')  # 采集日期 = 当期
            AND (t.B050038 IS NULL OR t.B050038 >= P_DATA_DATE - 1)
  ...
  SELECT
      ...
      CASE
          ...
          THEN SUBSTR(TO_CHAR(cust.B050012, 'YYYYMMDD'), 1, 6)
          ELSE NULL
      END AS CSNY,
      ...
      CASE
          ...
          THEN TO_CHAR(cust.B050029, 'YYYYMMDD')
          ELSE NULL
      END AS SHMDRQ,
