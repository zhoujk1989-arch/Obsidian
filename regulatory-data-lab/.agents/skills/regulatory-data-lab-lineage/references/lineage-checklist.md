...
- When SQL is available, run `scripts/extract_sql_lineage.py --engine auto` or explain why it was skipped.
- Check whether `gsp_available` is true. If false, record that parsing used sqlglot/regex fallback and treat coverage as weaker.
...
- GSP `candidate_gsp` edges are preferred over lighter fallback edges when duplicated.
