/*
校验目标：EAST5.0 融资租赁业务表（IE_005_511）GBase 存储过程草案运行后检查。
对应过程：PROC_EAST_IE_005_511_RZZLYWB
...
SELECT CJRQ, XDHTH, XDJJH, COUNT(*) AS dup_cnt
  FROM IE_005_511
 WHERE CJRQ = ?
...
SELECT COUNT(*) AS null_pk_count
  FROM IE_005_511
 WHERE CJRQ = ?
   AND (CJRQ IS NULL OR XDHTH IS NULL OR XDJJH IS NULL);
