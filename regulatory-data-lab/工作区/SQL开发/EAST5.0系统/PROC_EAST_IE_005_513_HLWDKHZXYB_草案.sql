...
- 补齐 4 个码值 CASE 转换：HZFLX（合作方类型）、HZFS（合作方式）、XYZT（协议状态）、XZBZ（限制标志）、HZFZJLB（合作方证件类别）
- 补齐合作方式复杂加工逻辑：UNNEST 行转列 → CASE 码值映射 → 按协议ID分组 '+' 拼接 → 其他-前缀置顶 → 全空默认'其他-其他'
...
BEGIN
        ...
        FROM (
            SELECT
                ...
                CASE
                    ...
                    WHEN F250007_SINGLE = '00' THEN '其他-其他'
    ...
    -- 对 TMP_HZFS 中"其他-"置顶做二次处理（若存在多个其他-项，合并最前）
    -- 因 GBase 不支持窗口函数对 GROUP_CONCAT 结果二次操作，
    -- 此处采用 COALESCE 方式：若 raw_concat 中已按 sort_key 排序，
    -- sort_key=0 的项在前，即"其他-"项已在最前，无需二次处理。
    ...
    SELECT
         ...
         * 码值转换：'01'→'有效'，'02'→'其他-待生效'，'03'→'其他-中止'，
         * '04'→'终结'，'05'→'撤销'，'06'→'其他-无效'，'00-xxx'→'其他-xxx' */
        CASE TRIM(src.F250014)
            ...
            WHEN TRIM(src.F250014) = '00' THEN '其他-其他'
         ...
         * 加工规则：
         *   若合作方类型为'00'[无合作方]，则赋值为'其他-无合作方'；
         *   否则按码值映射后 '+' 拼接，其他-置顶，全空→'其他-其他' */
        CASE
            WHEN TRIM(src.F250006) = '00' THEN '其他-无合作方'
        ...
        CASE TRIM(src.F250006)
            ...
            WHEN TRIM(src.F250006) = '00' THEN '其他-其他'
