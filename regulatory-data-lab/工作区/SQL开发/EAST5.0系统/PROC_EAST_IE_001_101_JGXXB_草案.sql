...
CREATE PROCEDURE `PROC_EAST_IE_001_101_JGXXB`(
    IN I_DATE VARCHAR(8)
)
BEGIN
  #声明变量
  ...
  LEFT JOIN (
      SELECT
          ...
          p0.A010014
      FROM T_1_1 p0
      WHERE p0.A010020 = TO_CHAR(P_LAST_MON_DT, 'YYYY-MM-DD')
        AND NOT EXISTS (
            SELECT 1
            FROM T_1_1 p_min
            WHERE p_min.A010020 = TO_CHAR(P_LAST_MON_DT, 'YYYY-MM-DD')
      ...
      WHERE emp.A030028 <= TO_CHAR(P_DATA_DATE, 'YYYY-MM-DD')
        AND NOT EXISTS (
            ...
            WHERE emp_rank.A030001 = emp.A030001
              AND emp_rank.A030028 <= TO_CHAR(P_DATA_DATE, 'YYYY-MM-DD')
              AND (
  ...
  LEFT JOIN (
      ...
      FROM T_4_1 g
      WHERE g.D010012 = TO_CHAR(P_DATA_DATE, 'YYYY-MM-DD')
        AND (
      ...
      FROM T_4_3 a
      WHERE a.D030015 = TO_CHAR(P_DATA_DATE, 'YYYY-MM-DD')
  ) b
