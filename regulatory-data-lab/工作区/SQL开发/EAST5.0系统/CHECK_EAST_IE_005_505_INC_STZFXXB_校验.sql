/*
校验目标：EAST5.0 受托支付信息表（IE_005_505_INC）GBase 存储过程草案运行后检查。
对应过程：PROC_EAST_IE_005_505_INC_STZFXXB
...
SELECT XDJJH, STZFDXZH, CJRQ, COUNT(*) AS dup_cnt
  FROM IE_005_505_INC
 WHERE CJRQ = ?
...
SELECT COUNT(*) AS null_pk_count
  FROM IE_005_505_INC
 WHERE CJRQ = ?
   AND (XDJJH IS NULL OR STZFDXZH IS NULL OR CJRQ IS NULL);
