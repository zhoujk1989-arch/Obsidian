/*
校验目标：EAST5.0 代理代销交易信息表（IE_009_905_INC）GBase 存储过程草案运行后检查。
对应过程：PROC_EAST_IE_009_905_INC_DLDXJYXXB
...
SELECT CJRQ, JYBH, COUNT(*) AS dup_cnt
  FROM IE_009_905_INC
 WHERE CJRQ = ?
 GROUP BY CJRQ, JYBH
HAVING COUNT(*) > 1;
...
SELECT COUNT(*) AS null_pk_count
  FROM IE_009_905_INC
 WHERE CJRQ = ?
   AND (CJRQ IS NULL OR JYBH IS NULL);
