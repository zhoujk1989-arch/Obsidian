/*
校验目标：EAST5.0 垫款登记表（IE_005_512）GBase 存储过程草案运行后检查。
对应过程：PROC_EAST_IE_005_512_DKDJB
...
SELECT CJRQ, XDHTH, XDJJH, YHTBH, KHTYBH, COUNT(*) AS dup_cnt
  FROM IE_005_512
 WHERE CJRQ = ?
...
SELECT COUNT(*) AS null_pk_count
  FROM IE_005_512
 WHERE CJRQ = ?
   AND (CJRQ IS NULL OR XDHTH IS NULL OR XDJJH IS NULL OR YHTBH IS NULL OR KHTYBH IS NULL);
