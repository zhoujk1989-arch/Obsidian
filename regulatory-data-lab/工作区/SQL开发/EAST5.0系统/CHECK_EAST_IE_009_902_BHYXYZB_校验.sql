/*
校验目标：EAST5.0 保函与信用证表（IE_009_902）GBase 存储过程草案运行后检查。
对应过程：PROC_EAST_IE_009_902_BHYXYZB
...
SELECT CJRQ, HTBH, XYZBZDM, COUNT(*) AS dup_cnt
  FROM IE_009_902
 WHERE CJRQ = ?
...
SELECT COUNT(*) AS null_pk_count
  FROM IE_009_902
 WHERE CJRQ = ?
   AND (CJRQ IS NULL OR HTBH IS NULL OR XYZBZDM IS NULL);
