...
表级取数与关联规则（原文摘录）：
### 2.1 表级规则（Excel第 525 行） 通过【信贷交易】的【分户账号】内关联已完成一表通转换的【个人信贷分户账】的【分户账号】，取【信贷交易】.【采集日期】为当月的数据
...
BEGIN
    ...
    SELECT
        ...
        /* 代办人姓名：信贷交易.代办人姓名 -> T_7_2.G020025；直接映射：取【信贷交易】.【代办人姓名】 */
        src.G020025 AS DBRXM,
        /* 代办人证件号码：信贷交易.代办人证件号码 -> T_7_2.G020027；直接映射：取【信贷交易】.【代办人证件号码】 */
        ...
        NULL AS DFKHLB,
        /* 代办人客户类别：需求字段未与目标 DDL 注释精确匹配，待确认 */
        NULL AS DBRKHLB,
        ...
        NULL AS GSFZJG,
        /* 交易渠道：信贷交易.交易渠道 -> T_7_2.G020024；代码转化：通过【信贷交易】.【交易渠道】转换。 若为'01'[柜面],则赋值为'柜面'; 若为'02'[ATM(自动柜员机)],则赋值为'ATM'; 若为'03'[VTM（远程视频柜员机）)],则赋值为'VTM'; 若为'04'[POS（销售终端）],则赋值为'POS'; 若为'05'[网银],则赋值为'网银'; 若为'06'[手机银行],则赋值为'手机银行'; 若为'07-XX'[第三方支付],则赋值为'第三方支付-XX'; 若为'0...；转换规则需人工补齐 CASE 分支 */
        src.G020024 AS JYQD,
        /* 代办人证件类别：信贷交易.代办人证件类型 -> T_7_2.G020026；直接映射：取【信贷交易】.【代办人证件类型】 */
        ...
        /* 备注：信贷交易.备注 -> T_7_2.G020032；直接映射：【信贷交易】.【备注】 */
        src.G020032 AS BBZ,
    ...
    WHERE 1 = 1
      /* TODO: 按《025_个人信贷分户账明细记录.md》补齐采集日期、当月数据、终态纳入和排除条件。 */;
