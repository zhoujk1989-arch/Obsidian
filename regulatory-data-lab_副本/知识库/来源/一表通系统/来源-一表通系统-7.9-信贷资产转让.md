...
- 本来源包用于沉淀一表通系统 `7.9 信贷资产转让` 的目标表结构证据。
- 本地原始材料已拆分入库：`原始材料/表结构/一表通系统/T_7_9-信贷资产转让-DDL-2026-04-27.sql`。
- 当前按数据表 ingest 初始化字段主档，不硬造报表业务口径页或字段级血缘页。
- 外部来源关联：待补充。

## 本地文件

- `原始材料/表结构/一表通系统/T_7_9-信贷资产转让-DDL-2026-04-27.sql`
...
- 外部关联页面：待补充
- 检索关键词：`一表通`、`7.9`、`信贷资产转让`、`T_7_9`

## Key Findings

- `T_7_9` 表注释为 `7.9信贷资产转让`，本次 DDL 解析字段数为 19。
- 本地 DDL 主键：`G090001` + `G090003` + `G090005` + `G090010` + `G090018`。
...
- 需补充 `T_7_9` 的加工 SQL、存储过程或视图定义，确认字段来源、上下游对象和字段级血缘。
