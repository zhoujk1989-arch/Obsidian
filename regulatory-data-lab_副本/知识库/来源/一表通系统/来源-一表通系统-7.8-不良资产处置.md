...
- 本来源包用于沉淀一表通系统 `7.8 不良资产处置` 的目标表结构证据。
- 本地原始材料已拆分入库：`原始材料/表结构/一表通系统/T_7_8-不良资产处置-DDL-2026-04-27.sql`。
...
- 外部关联页面：待补充
- 检索关键词：`一表通`、`7.8`、`不良资产处置`、`T_7_8`

## Key Findings

- `T_7_8` 表注释为 `7.8不良资产处置`，本次 DDL 解析字段数为 25。
...
- 需补充 `T_7_8` 的加工 SQL、存储过程或视图定义，确认字段来源、上下游对象和字段级血缘。
