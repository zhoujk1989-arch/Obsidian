...
DROP TABLE IF EXISTS `IE_004_407`;
CREATE TABLE `IE_004_407` (
  ...
  `JXFS`                   VARCHAR(60)      DEFAULT NULL COMMENT '计息方式；无法按枚举类型填报的，填写“XX”，XX为银行自定义的计息方式',
  `JXBZ`                   VARCHAR(3)       DEFAULT NULL COMMENT '计息标志',
  `JDBZ`                   VARCHAR(12)      DEFAULT NULL COMMENT '借贷标志',
  `NBFHZZH`                VARCHAR(60)      DEFAULT NULL COMMENT '内部分户账账号；PK',
  ...
  `ZHMC`                   VARCHAR(450)     DEFAULT NULL COMMENT '账户名称；非隐私，不做变形。如果为境内涉密机构的，填报为“*********”。',
  `BZ`                     VARCHAR(3)       DEFAULT NULL COMMENT '币种；PK',
  `DFYE`                   DECIMAL(20,2)    DEFAULT NULL COMMENT '贷方余额',
  ...
  `JFYE`                   DECIMAL(20,2)    DEFAULT NULL COMMENT '借方余额',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='内部分户账';
